...
from app.tables import format_pagos_table, format_saldos_table

_MENU_WORDS = {"menu", "menú", "hola", "hi", "inicio", "facturas", "deudas", "saldos"}
...
def handle_conversation(whatsapp_id: str, text: str) -> str:
    ...
    if normalized in _CANCEL_WORDS:
        ...
        return "Operación cancelada. Escribe hola para ver tus saldos."

    if normalized in _PAYMENTS_WORDS:
        payments = get_payments_by_cliente_id(client["id"], limit=10)
        return format_pagos_table(payments, cliente_nombre=nombre)

    if normalized in _MENU_WORDS or state == STATE_START:
        update_session(
            whatsapp_id,
            state=STATE_SHOW_INVOICES if pending else STATE_START,
            selected_invoice_id=None,
            selected_amount=None,
        )
        return (
            f"Hola {nombre}.\n\n"
            f"{format_saldos_table(pending, cliente_nombre=nombre)}"
        )

    if state == STATE_SHOW_INVOICES:
        if not pending:
            reset_session(whatsapp_id)
            return format_saldos_table([], cliente_nombre=nombre)
        chosen = _resolve_invoice_choice(text, pending)
        if chosen is None:
            return (
                "No entendí la selección.\n\n"
                + format_saldos_table(pending, cliente_nombre=nombre)
            )
        update_session(
            whatsapp_id,
    ...
    if state == STATE_CONFIRM_AMOUNT:
        if normalized in {"si", "sí", "s", "ok", "confirmar"}:
            ...
            return (
                f"Solicitud registrada: {number} por {float(amount):.2f}{ref}.\n"
                "El pago queda PENDIENTE hasta conectar el proveedor. "
                "Escribe hola para ver saldos o mis pagos para consultarlos."
            )
    ...
    reset_session(whatsapp_id)
    return "Escribe hola para ver tus saldos pendientes."
